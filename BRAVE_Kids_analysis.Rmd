...
uniquesToFasta(uniques,fout = "~/BRAVE_Kids/sepp-package/seqs.fasta", ids=sequences)
```

```{bash}

gzip ./sepp-package/seqs.fasta
```


```{bash}
/opt/sepp-package/run-sepp.sh sepp-package/test.frag test-gg
```

```{r, view tree file with ggtree}
seq.tree = read.tree(file = "~/PigLungs/pigs_placement.tog.relabelled.tre") %>%
  keep.tip(., sequences)
```
